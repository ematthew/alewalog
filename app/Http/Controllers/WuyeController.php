...
class WuyeController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->has('search_keywords')) {
            ...
            $offices = Wuye::where('asset_no', 'LIKE', "%$search_keywords%")
            ...
            $offices = Wuye::where('paid_amount', '>=', $paid_amount)
            ->where('grand_total', '!=', $paid_amount)
            ->sortable('pid', 'DESC')->paginate(20);
        }

        return view('wuye.index', compact('offices'));
    }
    ...
    public function previewAll(Request $request)
    {
        ...
        if (Auth::user()->user_type == 'super') {
            ...
            $offices = Wuye::whereIn('id', $office_ids)->orderBy('pid', 'DESC')->get();
            return view('wuye.preview', compact('offices'));
        } else {
    ...
    public function store(Request $request)
    {
        ...
        $office->save();
        return redirect('wuye')->with('success', 'office information has been created Successfully');
    }
    public function generatePID(){
        $pid = rand(10000, 1000000);
        $user = Wuye::where(['pid' => $pid])->first();
        if($user){
    ...
    {
        if (Auth::user()->user_type == 'super') {
            $office = Wuye::findOrFail($id);
            return view('wuye.edit', compact('office'));
        } else {
            // return 'you are not allow to view this page';
            return redirect()->back();
        }
    ...
    public function view(Request $request)
    {
        // body
        if (Auth::user()->user_type == 'super') {
            $office = Wuye::where('pid', $request->pid)->first();
            return view('wuye.show', compact('office'));
        } else {
