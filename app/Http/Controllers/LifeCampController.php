...
class LifeCampController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->has('search_keywords')) {
            ...
            $offices = LifeCamp::where('asset_no', 'LIKE', "%$search_keywords%")
            ...
            $offices = LifeCamp::where('grand_total', '!=', $paid_amount)
    ...
    public function previewAll(Request $request)
    {
        ...
        if (Auth::user()->user_type == 'super') {
            ...
            $offices = LifeCamp::whereIn('id', $office_ids)->orderBy('pid', 'DESC')->get();
    ...
    public function generatePID(){
        ...
        $user = LifeCamp::where(['pid' => $pid])->first();
        if($user){
    ...
    {
        if (Auth::user()->user_type == 'super') {
            $office = LifeCamp::findOrFail($id);
            return view('life_camp.edit', compact('office'));
        } else {
            // return 'you are not allow to view this page';
            return redirect()->back();
        }
    ...
    public function view(Request $request)
    {
        // body
        if (Auth::user()->user_type == 'super') {
            $office = LifeCamp::where('pid', $request->pid)->first();
            return view('life_camp.show', compact('office'));
        } else {
