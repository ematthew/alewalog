...
use App\Models\Gwarinpa;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\Redirect;

class GwarinpaController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->has('search_keywords')) {
            ...
            $offices = Gwarinpa::where('asset_no', 'LIKE', "%$search_keywords%")
            ...
            $offices = Gwarinpa::where('paid_amount', '>=', $paid_amount)
    ...
    public function previewAll(Request $request)
    {
        ...
        if (Auth::user()->user_type == 'super') {
            ...
            $offices = Gwarinpa::whereIn('id', $office_ids)->orderBy('pid', 'DESC')->get();
    ...
    public function store(Request $request)
    {
        ...
        $office->save();
        return redirect('gwarinpa')->with('success', 'office information has been created Successfully');
    }
    public function generatePID(){
        $pid = rand(10000, 1000000);
        $user = Gwarinpa::where(['pid' => $pid])->first();
        if($user){
    ...
    {
        if (Auth::user()->user_type == 'super') {
            $office = Gwarinpa::findOrFail($id);
            return view('gwarinpa.edit', compact('office'));
        } else {
            // return 'you are not allow to view this page';
            return redirect()->back();
        }
    ...
    public function view(Request $request)
    {
        // body
        if (Auth::user()->user_type == 'super') {
            $office = Gwarinpa::where('pid', $request->pid)->first();
            return view('gwarinpa.show', compact('office'));
        } else {
