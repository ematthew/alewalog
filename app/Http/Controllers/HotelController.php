...
class HotelController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->has('search_keywords')) {
            ...
            $search_keywords = trim($search_keywords);
                $offices = Hotel::where('asset_no', 'LIKE', "%$search_keywords%")
            ...
            $offices = Hotel::where('grand_total', '!=', $paid_amount)
    ...
    public function previewAll(Request $request)
    {
        ...
        if (Auth::user()->user_type == 'super') {
            ...
            $offices = Hotel::whereIn('id', $office_ids)->orderBy('pid', 'DESC')->get();
    ...
    public function generatePID(){
        ...
        $user = Hotel::where(['pid' => $pid])->first();
        if($user){
    ...
    {
        if (Auth::user()->user_type == 'super') {
            $office = Hotel::findOrFail($id);
            return view('hotel.edit', compact('office'));
        } else {
            // return 'you are not allow to view this page';
            return redirect()->back();
        }
    ...
    public function update($id, Request $request)
    {
        ...
        if (Auth::user()->user_type == 'super') {
            ...
            return redirect('hotel')->with('success', 'office information has been created Successfully');
        } else {
            // return 'you are not allow to view this page';
            return redirect()->back();
        }
