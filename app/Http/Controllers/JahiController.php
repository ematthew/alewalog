...
use App\Imports\JahiImport;

class JahiController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->has('search_keywords')) {
            ...
            $offices = Jahi::where('asset_no', 'LIKE', "%$search_keywords%")
                ->orWhere('prop_addr', 'LIKE', "%$search_keywords%")
                ->orWhere('occupant', 'LIKE', "%$search_keywords%")
                ->orWhere('pid', 'LIKE', "%$search_keywords%")
                ->orderBy('pid', 'DESC')
                ->paginate(20);
        } else {

            $offices = Jahi::where('paid_amount', '>=', $paid_amount)
            ->where('grand_total', '!=', $paid_amount)
            ->sortable('pid', 'DESC')->paginate(20);
        }

        return view('jahi.index', compact('offices'));
    }
    ...
    public function previewAll(Request $request)
    {
        ...
        if (Auth::user()->user_type == 'super') {
            ...
            $offices = Jahi::whereIn('id', $office_ids)->orderBy('pid', 'DESC')->get();
            return view('jahi.preview', compact('offices'));
        } else {
    ...
    */
    public function view(Request $request)
    {
        // body
        if (Auth::user()->user_type == 'super') {
            $office = Jahi::where('pid', $request->pid)->first();
            return view('jahi.show', compact('office'));
        } else {
    ...
    public function store(Request $request)
    {
        ...
        $office->save();
        return redirect('jahi')->with('success', 'office information has been created Successfully');
    }

    public function generatePID(){
        $pid = rand(10000, 1000000);
        $user = Jahi::where(['pid' => $pid])->first();
        if($user){
            return $this->generatePID();
        }
        return $pid;
    ...
    {
        if (Auth::user()->user_type == 'super') {
            $office = Jahi::findOrFail($id);
            return view('jahi.edit', compact('office'));
        } else {
            // return 'you are not allow to view this page';
            return redirect()->back();
        }
    ...
    public function bulkUpload(Request $request)
    {
        ...
        try {
            ...
            Excel::import(new JahiImport, $request->file('excel_file'));
            
            // \DB::commit();
            return'success' . 'Data imported successfully';
            // return redirect()->route('jahi.index')->with('success', 'Data imported successfully');
        } catch (\Exception $e) {
            // \DB::rollBack();
            return 'error'. 'Error importing data: ' . $e->getMessage();
