...
class GuduController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->has('search_keywords')) {
            ...
            $offices = Gudu::where('asset_no', 'LIKE', "%$search_keywords%")
            ...
            $offices = Gudu::where('grand_total', '!=', $paid_amount)
    ...
    public function previewAll(Request $request)
    {
        if (Auth::user()->user_type == 'super') {
            ...
            $offices = Gudu::whereIn('id', $office_ids)->orderBy('pid', 'DESC')->get();
    ...
    {
        if (Auth::user()->user_type == 'super') {
            $office = Gudu::findOrFail($id);
            return view('gudu.edit', compact('office'));
        } else {
    ...
    public function view(Request $request)
    {
        // body
        if (Auth::user()->user_type == 'super') {
            $office = Gudu::where('pid', $request->pid)->first();
