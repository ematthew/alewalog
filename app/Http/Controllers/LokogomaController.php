...
use App\Models\Lokogoma;
use Illuminate\Http\Request;
use App\Models\Office;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\Redirect;

class LokogomaController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->has('search_keywords')) {
            ...
            $offices = Lokogoma::where('asset_no', 'LIKE', "%$search_keywords%")
            ...
            $offices = Lokogoma::where('grand_total', '!=', $paid_amount)
    ...
    public function previewAll(Request $request)
    {
        ...
        if (Auth::user()->user_type == 'super') {
            ...
            $offices = Lokogoma::whereIn('id', $office_ids)->orderBy('pid', 'DESC')->get();
            return view('lokogoma.preview', compact('offices'));
        } else {
    ...
    public function store(Request $request)
    {
        ...
        $office->save();
        return redirect('lokogoma')->with('success', 'office information has been created Successfully');
    }

    public function generatePID(){
        $pid = rand(10000, 1000000);
        $user = Lokogoma::where(['pid' => $pid])->first();
        if($user){
    ...
    public function view(Request $request)
    {
        // body
        if (Auth::user()->user_type == 'super') {
            $office = Lokogoma::where('pid', $request->pid)->first();
            return view('gudu.show', compact('office'));
        } else {
    ...
    {
        if (Auth::user()->user_type == 'super') {
            $office = Lokogoma::findOrFail($id);
            return view('lokogoma.edit', compact('office'));
        } else {
            // return 'you are not allow to view this page';
            return redirect()->back();
        }
