...
class JabiController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->has('search_keywords')) {
            ...
            $offices = Jabi::where('asset_no', 'LIKE', "%$search_keywords%")
            ...
            $offices = Jabi::where('paid_amount', '>=', $paid_amount)
    ...
    public function previewAll(Request $request)
    {
        ...
        if (Auth::user()->user_type == 'super') {
            ...
            $offices = Jabi::whereIn('id', $office_ids)->orderBy('pid', 'DESC')->get();
            return view('jabi.preview', compact('offices'));
        } else {
    ...
    */
    public function view(Request $request)
    {
        // body
        if (Auth::user()->user_type == 'super') {
            $office = Jabi::where('pid', $request->pid)->first();
    ...
    public function store(Request $request)
    {
        ...
        $office->save();
        return redirect('jabi')->with('success', 'office information has been created Successfully');
    }

    public function generatePID(){
        $pid = rand(10000, 1000000);
        $user = Jabi::where(['pid' => $pid])->first();
        if($user){
    ...
    {
        if (Auth::user()->user_type == 'super') {
            $office = Jabi::findOrFail($id);
            return view('jabi.edit', compact('office'));
        } else {
            // return 'you are not allow to view this page';
            return redirect()->back();
        }
